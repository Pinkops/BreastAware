...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-ivory text-charcoal flex">
      {/* BA-040: Skip link for keyboard users */}
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:fixed focus:top-3 focus:left-3 focus:z-[100] focus:px-4 focus:py-2 focus:rounded-xl focus:bg-forest focus:text-white focus:shadow-lg"
      >
      ...
      <aside className="hidden lg:flex lg:flex-col w-64 shrink-0 border-r border-border bg-white/70 backdrop-blur-sm sticky top-0 h-screen">
        <div className="px-6 py-6 border-b border-border">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-charcoal/60 mt-0.5 tracking-wide uppercase">Private companion</p>
            </div>
          </div>
        </div>
          ...
          {sideNav.map((item) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-colors focus-visible:outline focus-visible:outline-2 focus-visible:outline-forest/40 ${
                    isActive
        ...
        <div className="px-5 py-4 border-t border-border">
          ...
          <p className="text-[11px] text-charcoal/55 mt-1 leading-snug">
      ...
      <div className="flex-1 flex flex-col min-w-0 pb-20 lg:pb-0">
        <header className="lg:hidden sticky top-0 z-30 bg-ivory/95 backdrop-blur border-b border-border px-4 py-3 flex items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-charcoal/60 truncate">{location.pathname === '/' ? 'Your private space' : ''}</p>
          </div>
        </header>

        {/* BA-028: Offline banner */}
        {!isOnline && (
          <div className="bg-amber-100 border-b border-amber-200 text-amber-900 text-xs px-4 py-2 flex items-center gap-2" role="status" aria-live="polite">
            ...
            <span>You are offline — your private data is not available offline. Changes will sync when you are back online.</span>
          </div>
        )}
      ...
      {/* Mobile bottom nav - BA-041: 44px touch targets */}
      <nav
          ...
          {mobileNav.map((item) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex flex-col items-center justify-center gap-0.5 px-2 py-2 min-w-[3.75rem] min-h-[44px] rounded-xl text-[10px] font-medium focus-visible:outline focus-visible:outline-2 focus-visible:outline-forest/40 ${
                    isActive ? 'text-forest bg-forest/8' : 'text-charcoal/60'
                  }`
                }
              >
